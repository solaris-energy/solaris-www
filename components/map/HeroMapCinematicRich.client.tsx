...
const HYBRID_STYLE: maplibregl.StyleSpecification = {
  ...
  sources: {
    "sat-low": {
      ...
      tileSize: 256,
      maxzoom: 11,
      attribution: "© EOX · Sentinel-2 cloudless · ESA",
    },
    "sat-high": {
      type: "raster",
      tiles: [
...
export default function HeroMapCinematicRich() {
  ...
  useMotionValueEvent(scrollYProgress, "change", (p) => {
    ...
    if (map.getLayer("pv-fill")) {
      ...
      map.setPaintProperty("pv-edge", "line-opacity", arrayT * 0.7);
    }
  });
    ...
    >
      <div className="sticky top-0 h-screen w-full overflow-hidden border-y border-[color:var(--color-bg-edge)] bg-[color:var(--color-bg-void)]">
        ...
        <div className="pointer-events-none absolute inset-0 mx-auto flex w-full max-w-[1440px] flex-col justify-end px-6 pb-12 md:px-12 md:pb-20 xl:px-16">
          ...
          {/* Beat tick HUD */}
          <div
            className="absolute top-6 right-6 flex items-center gap-2 font-mono text-[10px] tracking-[0.08em] text-[color:var(--color-fg-tertiary)] uppercase md:top-8 md:right-12 xl:right-16"
            style={{ fontFamily: "var(--font-mono)" }}
          >
            <span>
              Beat 0
              {Math.min(beats.length, Math.floor(progress * (beats.length - 1)) + 1)} /
              0{beats.length}
            </span>
